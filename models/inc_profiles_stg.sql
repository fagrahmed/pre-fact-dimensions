...
{{ config(
    ...
    pre_hook=[
        "{% if target.schema == 'dbt-dimensions' and source('dbt-dimensions', 'inc_profiles_stg') is not none %}TRUNCATE TABLE {{ source('dbt-dimensions', 'inc_profiles_stg') }};{% endif %}"
    ]
...
{% if is_incremental() and table_exists and stg_table_exists %}
    WHERE (wp._airbyte_emitted_at::timestamptz AT TIME ZONE 'UTC' + INTERVAL '3 hours') > COALESCE((SELECT max(loaddate::timestamptz) FROM {{ source('dbt-dimensions', 'inc_profiles_dimension') }}), '1900-01-01'::timestamp)
        OR (p._airbyte_emitted_at::timestamptz AT TIME ZONE 'UTC' + INTERVAL '3 hours') > COALESCE((SELECT max(loaddate::timestamptz) FROM {{ source('dbt-dimensions', 'inc_profiles_dimension') }}), '1900-01-01'::timestamp)
{% endif %}
